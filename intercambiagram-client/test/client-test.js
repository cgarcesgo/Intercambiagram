...
const intercambiagram = require('../')
const fixtures = require('./fixtures')

let options = {
  endpoints: {
    pictures: 'http://intercambiagram.test/picture',
    users: 'http://intercambiagram.test/user',
    auth: 'http://intercambiagram.test/auth'
  }
}

test.beforeEach(t => {
  t.context.client = intercambiagram.createClient(options)
})
...
test('saveUser', async t => {
  ...
  let newUser = {
    ...
    name: user.name,
    email: 'user@intercambiagram.test',
    password: 'pl4tzi'
  }
